...
def get_state(modules):
    ...
    weight_grad = []
    bias_grad = []
    for n, m in modules.named_modules():
        if len(m._parameters) != 0:
            weight.append(m._parameters['weight'].detach().view(-1))
            weight_grad.append(m._parameters['weight'].grad.detach().view(-1).abs())
            try:
                bias.append(m._parameters['bias'].detach().view(-1))
                bias_grad.append(m._parameters['bias'].grad.detach().view(-1).abs())
            except:
                pass
    ...
    weight_grad_mean = weight_grad.mean().item()
    bias_grad_mean = bias_grad.mean().item()

    return [weight_mean, bias_mean, weight_grad_mean, bias_grad_mean]


def update_state(model, global_feature):
    node_features = []
    # embedding module
    embedding_node = get_state(model.roberta.embeddings)
    # print(embedding_node)
    embedding_node.extend(global_feature)
    node_features.append(embedding_node)
    # bert layer module
    for layer in model.roberta.encoder.layer:
        att_node = get_state(layer.attention.self)
        self_out_node = get_state(layer.attention.output)
        inter_node = get_state(layer.intermediate)
        out_node = get_state(layer.output)
        add_1_node = list(np.array(att_node) + np.array(self_out_node))
        add_2_node = list(np.array(add_1_node) + np.array(out_node))
        att_node.extend(global_feature)
        self_out_node.extend(global_feature)
        inter_node.extend(global_feature)
        out_node.extend(global_feature)
        add_1_node.extend(global_feature)
        add_2_node.extend(global_feature)
        node_features.append(att_node)
        node_features.append(self_out_node)
        node_features.append(add_1_node)
        node_features.append(inter_node)
        node_features.append(out_node)
        node_features.append(add_2_node)
    # pooler
    pooler_node = get_state(model.classifier.dense)
    pooler_node.extend(global_feature)
    node_features.append(pooler_node)
    # classifier
    classifier_node = get_state(model.classifier.out_proj)
    classifier_node.extend(global_feature)
    node_features.append(classifier_node)
...
class Trainer:
    ...
    def train(
        ...
        **kwargs,
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    curr_tr_loss = self.training_step(model, inputs)
                    tr_loss += curr_tr_loss
                    tr_loss_list.append(curr_tr_loss.item())
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    # if not self.deepspeed and self.state.global_step < 100:
                    #     # pass
                    #     self.lr_scheduler.step()
                    #     if self.state.global_step == 99:
                    #         tr_loss_list = []
                    #         self.evaluate()
                    #         prev_eval_loss = self.eval_loss
                    ...
                    # print(self.args.eval_steps, self.control.should_save)

                    if self.state.global_step == 1:
                        # obtain state
                        metrics = self.evaluate()
                        if model.config.finetuning_task == 'mrpc':
                            if self.state.best_metric is None:
                                self.state.best_metric = {
                                    "eval_loss": None,
                                    "eval_accuracy": None,
                                    "eval_combined_score": None,
                                    "eval_f1": None
                                }
                                for k in self.state.best_metric.keys():
                                    self.state.best_metric[k] = metrics[k]
                            prev_score = metrics['eval_accuracy']
                        elif model.config.finetuning_task == 'cola':
                            if self.state.best_metric is None:
                                self.state.best_metric = {
                                    "eval_matthews_correlation": None,
                                    "eval_loss": None,
                                }
                                for k in self.state.best_metric.keys():
                                    self.state.best_metric[k] = metrics[k]
                            prev_score = metrics['eval_matthews_correlation']
                        elif model.config.finetuning_task == 'stsb':
                            if self.state.best_metric is None:
                                self.state.best_metric = {
                                    'eval_pearson': None,
                                    'eval_spearmanr': None,
                                ...
                                for k in self.state.best_metric.keys():
                                    self.state.best_metric[k] = metrics[k]
                            prev_score = metrics['eval_pearson']
                        else:
                            if self.state.best_metric is None:
                                self.state.best_metric = {
                                    'eval_accuracy': None,
                                ...
                                for k in self.state.best_metric.keys():
                                    self.state.best_metric[k] = metrics[k]
                            prev_score = metrics['eval_accuracy']
                        # curr_eval_loss = self.eval_loss
                        state_tr_loss = np.array(tr_loss_list).mean()
                        for param_group in self.optimizer.param_groups:
                            lr = param_group['lr']
                        state = update_state(model, global_feature)
                        state = np.array(state)
                        action, action_log_prob = agent.select_action(state)
                        # execute action
                        for param_group in self.optimizer.param_groups:
                            param_group['lr'] = param_group['lr'] * (1 + action)
                        tr_loss_list = []
                    if (self.state.global_step - 1) % 10 == 0 and self.state.global_step != 1:
                        if np.random.random() < 0.2:
                            metrics = self.evaluate()
                            if model.config.finetuning_task == 'mrpc':
                                if metrics['eval_accuracy'] > self.state.best_metric['eval_accuracy']:
                                    for k in self.state.best_metric.keys():
                                        self.state.best_metric[k] = metrics[k]
                                    self._save_checkpoint(model, trial, metrics=metrics)
                                    if self.state.best_model_checkpoint is not None:
                                        shutil.rmtree(self.state.best_model_checkpoint)
                                    checkpoint_folder = f"checkpoint-{self.state.global_step}"
                                    run_dir = self.args.output_dir
                                    self.state.best_model_checkpoint = os.path.join(run_dir, checkpoint_folder)
                                curr_score = metrics['eval_accuracy']
                            elif model.config.finetuning_task == 'cola':
                                if metrics['eval_matthews_correlation'] > self.state.best_metric['eval_matthews_correlation']:
                                    for k in self.state.best_metric.keys():
                                        self.state.best_metric[k] = metrics[k]
                                    self._save_checkpoint(model, trial, metrics=metrics)
                                    if self.state.best_model_checkpoint is not None:
                                    ...
                                    run_dir = self.args.output_dir
                                    self.state.best_model_checkpoint = os.path.join(run_dir, checkpoint_folder)
                                curr_score = metrics['eval_matthews_correlation']
                            elif model.config.finetuning_task == 'stsb':
                                if metrics['eval_pearson'] > self.state.best_metric['eval_pearson']:
                                    for k in self.state.best_metric.keys():
                                        self.state.best_metric[k] = metrics[k]
                                    self._save_checkpoint(model, trial, metrics=metrics)
                                    if self.state.best_model_checkpoint is not None:
                                    ...
                                    run_dir = self.args.output_dir
                                    self.state.best_model_checkpoint = os.path.join(run_dir, checkpoint_folder)
                                curr_score = metrics['eval_pearson']
                            else:
                                if metrics['eval_accuracy'] >= self.state.best_metric['eval_accuracy']:
                                    for k in self.state.best_metric.keys():
                                        self.state.best_metric[k] = metrics[k]
                                    ...
                                    self.state.best_model_checkpoint = os.path.join(run_dir, checkpoint_folder)
                                curr_score = metrics['eval_accuracy']
                            reward = 10 * (curr_score - prev_score) * (curr_score + prev_score)
                            if model.config.finetuning_task == 'mrpc' or model.config.finetuning_task == 'qqp':
                                if curr_score > 0.8:
                                    reward = reward * 10
                            elif model.config.finetuning_task == 'rte' or model.config.finetuning_task == 'mnli':
                                if curr_score > 0.8:
                                    reward = reward * 10
                            elif model.config.finetuning_task == 'sst2' or model.config.finetuning_task == 'qnli':
                                if curr_score > 0.85:
                                    reward = reward * 10
                            elif model.config.finetuning_task == 'cola':
                                if curr_score > 0.55:
                        ...
                        global_feature = [state_tr_loss, lr]
                        next_state = update_state(model, global_feature)
        ...
        if model.config.finetuning_task == 'mrpc':
            if metrics['eval_loss'] <= self.state.best_metric['eval_loss']:
                for k in self.state.best_metric.keys():
                    self.state.best_metric[k] = metrics[k]
            self._save_checkpoint(model, trial, metrics=metrics)
            if self.state.best_model_checkpoint is not None:
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch):
        ...
        metrics = None
        # print(self.control.should_evaluate, self.control.should_save)
